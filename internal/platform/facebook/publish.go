...
func PublishVideo(ctx context.Context, logger *logx.Logger, req PublishRequest) error {
	...
	// 清理多余标签页
	if err := chromedputil.CleanExtraTabs(tabCtx, logger, "FB1"); err != nil {
		// 连不上浏览器时必须立即返回：chromedp 首次分配已失败并留下不一致状态，
		// 继续调用 chromedp.Run 会 panic(close of closed channel) 并终止整个进程。
		return fmt.Errorf("FB1 %v", err)
	}

	defer chromedputil.CloseTabsAndStopProfile(ctx, tabCtx, logger, req.ProfileID, req.UndetectableHost, req.UndetectablePort, req.WebsocketURL, "FB7")
